...
from wkmigrate.trigger_translators.parsers import parse_cron_expression, _get_hourly_cron_expression, \
    _get_daily_cron_expression, _get_weekly_cron_expression, _get_monthly_cron_expression, _get_week_day
...
class TestTriggerTranslator:
    ...
    @pytest.mark.parametrize('trigger_definition, expected_result', [
        ({'properties': {
        ...
        ({'properties': {
                'recurrence': {'frequency': 'Day', 'interval': 2, 'schedule': {'hours': [9], 'minutes': [23]}},
                'time_zone': 'Eastern Standard Time'
            }},
        ...
        ({'properties': {
                'recurrence': {'frequency': 'Week', 'interval': 1,
                               'schedule': {'week_days': ['Sunday', 'Wednesday'], 'hours': [9], 'minutes': [15]}},
                'time_zone': 'Eastern Standard Time'
            }},
        ...
        ({'properties': {
                'recurrence': {'frequency': 'Month', 'interval': 1,
                               'schedule': {'days': [3, 12], 'hours': [9], 'minutes': [15]}},
                'time_zone': 'Eastern Standard Time'
            }},
            {'quartz_cron_expression': '0 15 9 3,12 * ?', 'timezone_id': 'UTC'}),
        ({'properties': {
                'recurrence': {'frequency': 'Month', 'interval': 5,
                               'schedule': {'days': [3, 12], 'hours': [9], 'minutes': [15]}},
                'time_zone': 'Eastern Standard Time'
            }},
            {'quartz_cron_expression': '0 15 9 3,12 * ?', 'timezone_id': 'UTC'}),
    ...
    @pytest.mark.parametrize('trigger_definition, expected_error_message', [
    ...
    @pytest.mark.parametrize('extra_property, extra_value', [
    ...
    @pytest.mark.parametrize("recurrence, expected_result", [
        (None, None),
        ({'frequency': IntervalType.HOUR, 'interval': 2}, '0 0 */2 * * ?'),
        ({'frequency': IntervalType.DAY, 'interval': 1, 'schedule': {'minutes': [30], 'hours': [9]}}, '0 30 9 */1 * ?'),
        ({'frequency': IntervalType.WEEK,
          'schedule': {'minutes': [0], 'hours': [8], 'week_days': ['Monday', 'Wednesday']}}, '0 0 8 ? * 2,4'),
        ({'frequency': IntervalType.MONTH, 'schedule': {'minutes': [15], 'hours': [10], 'days': [1, 15]}},
         '0 15 10 1,15 * ?'),
    ])
    def test_parse_cron_expression(self, recurrence, expected_result):
        assert parse_cron_expression(recurrence) == expected_result

    @pytest.mark.parametrize("num_intervals, expected_result", [
        (1, '0 0 */1 * * ?'),
        (3, '0 0 */3 * * ?'),
    ])
    ...
    @pytest.mark.parametrize("num_intervals, schedule, expected_result", [
        (1, None, '0 0 0 */1 * ?'),
    ...
    @pytest.mark.parametrize("schedule, expected_result", [
        (None, '0 0 0 ? * 1'),
        ({'minutes': [0], 'hours': [8], 'week_days': ['Monday', 'Friday']}, '0 0 8 ? * 2,6'),
    ])
    ...
    @pytest.mark.parametrize("week_day, expected_result", [
        ('Sunday', '1'),
        ('Monday', '2'),
        ('Tuesday', '3'),
        ('Wednesday', '4'),
    ...
    @pytest.mark.parametrize("recurrence", [
        {'frequency': IntervalType.WEEK, 'interval': 2, 'schedule': {'week_days': ['Monday']}},
