...
class TestActivityParsers:
    ...
    @pytest.mark.parametrize("column_mapping, expected_result, context", [
        (None, None, does_not_raise()),
        ({}, None, does_not_raise()),
        ({'mappings': [{'source': 'col1', 'sink': 'col1'}]}, [{'source': 'col1', 'sink': 'col1'}], does_not_raise())
    ])
    ...
    @pytest.mark.parametrize("query_timeout_definition, expected_result, context", [
    ...
    @pytest.mark.parametrize("query_isolation_level, expected_result, context", [
        (None, 'READ_COMMITTED', does_not_raise()),
        ({}, 'READ_COMMITTED', does_not_raise()),
        ({'isolation_level': 'ReadCommitted'}, IsolationLevel.ReadCommitted.value, does_not_raise()),
        ({'isolation_level': 'Serializable'}, IsolationLevel.Serializable.value, does_not_raise())
    ])
    ...
    @pytest.mark.parametrize("for_each_items, expected_result, context", [
    ...
    @pytest.mark.parametrize("policy_definition, expected_result, context", [
        (None, {}, does_not_raise()),
        ({'timeout': '0.01:30:00', 'retry': 3, 'retry_interval_in_seconds': 60},
         {'timeout_seconds': 5400, 'max_retries': 3, 'min_retry_interval_millis': 60000},
         does_not_raise()),
    ])
    ...
    @pytest.mark.parametrize("dependencies, expected_result, context", [
        (None, None, does_not_raise()),
        ([{'activity': 'Task1', 'dependencyConditions': ['Succeeded']}],
         [{'task_key': 'Task1', 'outcome': None}],
         does_not_raise()),
        ([{'activity': 'Task1', 'dependencyConditions': ['Succeeded', 'Completed']}],
         None,
    ...
    @pytest.mark.parametrize("notebook_parameters, expected_result", [
        (None, None),
        ({'param1': 'value1', 'param2': 'value2'}, {'param1': 'value1', 'param2': 'value2'}),
        ({'param1': 'value1', 'param2': {'expression': '@this()'}}, {'param1': 'value1', 'param2': ''})
    ])
    ...
    @pytest.mark.parametrize('condition_expression, expected_result, context', [
        ({'expression': '@equals(1, 1)'}, None, pytest.raises(ValueError)),
        ({'value': '@equals(1, 1)'}, {'op': 'EQUAL_TO', 'left': '1', 'right': '1'}, does_not_raise()),
        ({'value': '@greater(2, 1)'}, {'op': 'GREATER_THAN', 'left': '2', 'right': '1'}, does_not_raise()),
    ])
    ...
    @pytest.mark.parametrize("timeout_string, expected_result", [
        ('0.00:05:00', 300),
        ('1.02:30:00', 95400),
    ])
    ...
    @pytest.mark.parametrize("array_string, expected_result", [
        ("1,2,3", '["1","2","3"]'),
        ("'a','b','c'", '["a","b","c"]'),
    ])
