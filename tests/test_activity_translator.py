...
class TestActivityTranslator:
    ...
    @pytest.mark.parametrize('activity_definition, expected_result', [
        (None, None),
        ([], []),
        ([{
            ...
            'policy': {'timeout': '7.00:00:00', 'retry': 3, 'retry_interval_in_seconds': 30},
        ...
        ([{
            ...
            'description': 'Test activity',
            'policy': {'timeout': '7.00:00:00', 'retry': 3, 'retry_interval_in_seconds': 30},
            'depends_on': [{'activity': 'PreviousActivity', 'dependency_conditions': ['Succeeded']}],
            'notebook_path': '/path/to/notebook'},
         {
            'type': 'DatabricksNotebook',
            'description': 'Test activity',
    ...
    @pytest.mark.parametrize('activity_definition, expected_result, context', [
        ({
            'type': 'DatabricksNotebook',
            'name': 'Activity1',
            'description': 'Test activity',
    ...
    @pytest.mark.parametrize('activity_properties, expected_result', [
        ({'type': 'DatabricksNotebook', 'notebook_path': '/path/to/notebook'},
         {'notebook_task': {'notebook_path': '/path/to/notebook'}}),
        ({'type': 'DatabricksNotebook', 'notebook_path': '/path/to/notebook', 'base_parameters':
                {'param_name': 'param_value'}},
         {'notebook_task': {'notebook_path': '/path/to/notebook', 'base_parameters': {'param_name': 'param_value'}}})
    ])
